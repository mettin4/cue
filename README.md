...
On testnet there is no real on ramp, so the dashboard's Add Money button and the `add_funds` tool grant a small, fixed amount of test funds from a shared demo pool. It is capped so a signup loop cannot drain the pool: a fixed grant per request, a lifetime total per account, and a short wait between grants, all env tunable, with defaults of ten dollars per request, thirty per account, and five minutes between. Every grant also passes the same treasury floor and daily cap as a send, and when the pool cannot cover one the Add action is shown disabled with the reason rather than failing on press. The funds have no real value. On mainnet this is where funding through Circle would sit; the test grant is the placeholder until then.
...
src/
  ...
  lib/
    ...
    cue/                  send, cancel, claim, requests, contacts, schedules, limits, debts, summary, fund, escrow
...
## Interactive cards in Claude

Cue is built on the MCP Apps extension, so every tool renders an interactive card in Claude rather than a wall of text. A send shows the amount, the recipient and when it can be collected; a balance shows the number with recent activity; a debts view lists who owes what. The card carries the structured data, and the plain text underneath is only a fallback for clients that do not render it.

For anything that moves money, the card is also the control. The first call returns a preview with an **Approve** button, and nothing happens until it is pressed. Approving calls the tool back with a single signed, one time confirmation token, and the server rebuilds every detail from that token rather than trusting anything the card sends back. So the confirmation you see is exactly the one that executes, and it cannot run twice.

![A Cue confirmation card in Claude](docs/card.png)
...
Sign in links are generated by us with `admin.generateLink` and delivered through Resend from our own verified domain, `hello@cueworks.net`, all behind one function, [`sendMagicLink`](src/lib/auth/magic-link.ts), with a branded template. For links to resolve on production the Supabase dashboard needs the site URL added under Authentication, URL Configuration.
...
- The MCP server with twenty actions, every one rendering an interactive card in Claude: add test funds, on-chain escrow, send money, call a send back, request money, split a total, schedule a recurring payment, manage schedules, save a contact, list contacts, get balance, a spending summary, set a spending limit, track a debt, list debts, settle a debt, remind about a debt, get history (payments or requests), check collect status and resend the collection link. Send, cancel, request, split, scheduling, deleting a schedule, loosening a limit and settling by sending require an explicit Approve on the card before anything happens. Contact names resolve to emails, and never to a guessed address. Verified against the deployed endpoint.
- Email magic link sign in through Supabase Auth, with sign in links sent from our own domain, `hello@cueworks.net`, through Resend. No passwords, sessions in httpOnly cookies, sign out, a rate limited link request, existing accounts linked by email, and a signed out dashboard that invites you in rather than locking the door. Collecting money mints a scoped session that can view but not act, so the effortless part stays effortless. Verified end to end on the deployed site: sign up, sign in, an existing account keeping its data, and a brand new recipient collecting into a scoped session.
- Add test funds on testnet, capped per request, per account and by a cooldown, with the button disabled and a reason shown when the pool cannot cover a grant. See [Adding test funds](#adding-test-funds).
- On-chain escrow on Arc through Circle Contracts. `CueEscrow` is deployed and exercised with a real lock, reclaim and withdraw; see [On-chain escrow](#on-chain-escrow-circle-contracts) for the address and transaction hashes.
...
- **No mainnet.** Everything runs on Arc testnet with test funds that have no real value, and the keys in use are testnet keys that will be rotated before any mainnet use.
- **No fiat off ramp.** Cashing out to a bank is documented as an intended mainnet design through a regulated payout partner, not built, and the demo never simulates a bank payout. See [What a recipient can do with the money](#what-a-recipient-can-do-with-the-money).
- **The connect link is a bearer credential in the URL, not OAuth.** Anyone who holds it can act as that account, which is why it can be revoked and regenerated at any time. A proper OAuth flow for the connector is future work.
- **Escrow is a demonstrated alternative, not the default path.** The default send keeps funds in the treasury and treats a call back as a database update; only an explicit escrow action goes through the contract.
...
| `CUE_ESCROW_ADDRESS` | The deployed CueEscrow contract on Arc, required for the escrow tool | Printed by `scripts/escrow/deploy.mjs` |
| `CUE_ESCROW_UNLOCK_SECONDS` | How long an escrow deposit stays locked before it can be withdrawn | Optional, defaults to 60 |
| `ARC_USDC_ADDRESS` | ERC-20 USDC address on Arc, used by the escrow contract | Optional, defaults to `0x3600000000000000000000000000000000000000` |
| `ARC_RPC_URL` | Arc testnet RPC, used only to read a transaction receipt for the escrow lock id | Optional, defaults to `https://rpc.testnet.arc.network` |
| `CUE_FUND_AMOUNT_USDC` | Test funds granted per Add Money request, in dollars | Optional, defaults to 10 |
| `CUE_FUND_ACCOUNT_CAP_USDC` | Lifetime test funds cap per account, in dollars | Optional, defaults to 30 |
| `CUE_FUND_COOLDOWN_SECONDS` | Wait between test funds grants | Optional, defaults to 300 |
| `CUE_TREASURY_FLOOR_USDC` | Balance the demo pool can never drop below | Optional, defaults to 5 |
| `CUE_TREASURY_DAILY_CAP_USDC` | Cap on total demo outflow per day | Optional, defaults to 50 |
| `CUE_TREASURY_ALERT_USDC` | Available balance below which a low pool alert is emailed | Optional, defaults to 10 |
| `CUE_ALERT_EMAIL` | Where low pool alerts are sent | Optional, alerts are skipped when unset |

**Set up the database**

Apply the schema. It creates the tables (`users`, `transactions`, `scheduled_payments`, `email_logs`, `connect_tokens`, `contacts`, `requests`, `debts`, `used_confirmations` and `escrow_deposits`) with row level security on and grants for the service role. It is safe to run more than once.
